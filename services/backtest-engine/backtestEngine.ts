...
export interface BacktestResult {
  ...
  returnsSeries: Array<{ timestamp: number; return: number }>;
  tradeLog: Trade[];
  summaryStats: {
    totalReturn: number;
    cagr: number;
...
export class BacktestEngine {
  ...
  private executeStrategy(code: string, data: OHLCV[]): Array<'buy' | 'sell' | 'hold'> {
    try {
      // 创建安全的执行环境
  ...
  private executeBacktest(
    ...
    const returnsSeries: Array<{ timestamp: number; return: number }> = [];
    ...
    // 计算统计指标
    const summaryStats = this.calculateStats(
      equityCurve,
      returnsSeries,
      trades,
      initialCapital
  ...
  private calculateDrawdown(
    ...
    const drawdownSeries: Array<{ timestamp: number; drawdown: number }> = [];
